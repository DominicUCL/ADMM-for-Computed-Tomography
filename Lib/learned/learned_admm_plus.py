...
Run evaluate_learned_admm_plus.py to evaluate the trained parameters."""
...
with tf.name_scope('tomography'):
    ...
    for i in range(n_iter):
        ...
        with tf.variable_scope('admm_iterations_{}'.format(i)):
            ...
            update2 = tf.concat([x,odl_op_layer_adjoint(m[..., 1:2]/tau + odl_op_layer(x[..., 1:2]) - z[..., 1:2])*tau],axis=-1)
            ...
            update = tf.concat([sigma*(odl_op_layer(x[...,0:1])+m[..., 1:2]/sigma),y_rt],axis=-1)
...
with tf.name_scope('loss'):
    ...
    psnr = -10 * tf.log(loss) / tf.log(10.0)
...
writer = tf.summary.FileWriter('./learned_admm_plus', sess.graph)
